...
class _SearchResultsScreenState extends State<SearchResultsScreen>
  ...
  void _incrementPassengers() {
    if (passengerCount < 5) setState(() => passengerCount++);
  }
  ...
  Future<void> _showBookingOptionsModal(
    ...
    if (configResult != null && mounted) {
      _navigateToSeatSelection(
        ...
        returnProgram:
            configResult['returnProgram'], // Sera null si isRoundTrip était false
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.searchParams != null) {
      titrePage =
          "${widget.searchParams!['depart']} ➔ ${widget.searchParams!['arrivee']}";
      sousTitres = [
              ...
              itemBuilder: (context, index) {
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final double endDelay = (startDelay + 0.5).clamp(
                        0.0,
                        1.0,
                      );
          ...
          Container(
            ...
            child: ElevatedButton(
              onPressed: () {
                String searchDate =
                    widget.searchParams?['date'] ??
                    program.dateDepart.split(' ')[0];
                String timeOnly = program.heureDepart;
                String correctDateDepart = "$searchDate $timeOnly";
                ProgramModel programCorrige = program.copyWith(
                  dateDepart: correctDateDepart,
                );
...
class _BookingConfigModalContent extends StatefulWidget {
  ...
  @override
  State<_BookingConfigModalContent> createState() =>
      _BookingConfigModalContentState();
}

class _BookingConfigModalContentState
    extends State<_BookingConfigModalContent> {
  ...
  Future<void> _fetchReturnTrips(DateTime date) async {
    ...
    try {
      ...
      String formatCity(String city) =>
          "${city.split(',')[0].trim()}, Côte d'Ivoire";
  ...
  Future<void> _pickReturnDate() async {
    DateTime departureDate =
        DateTime.tryParse(widget.program.dateDepart) ?? DateTime.now();
    DateTime initialDate =
        selectedReturnDate ?? departureDate.add(const Duration(days: 1));
      ...
      child: SafeArea(
        ...
        child: Column(
          ...
          children: [
            ...
            // RÉCAP ALLER
            Text(
              "Aller : ${widget.program.villeDepart} ➔ ${widget.program.villeArrivee}",
              style: const TextStyle(
                fontSize: 13,
                fontWeight: FontWeight.bold,
                color: Colors.grey,
                ...
                children: [
                  ...
                  Text(
                    DateFormat("d MMM yyyy", "fr_FR").format(
                      DateTime.parse(widget.program.dateDepart.split(' ')[0]),
                    ),
                    style: const TextStyle(
                      fontWeight: FontWeight.bold,
                      color: AppColors.primary,
            ...
            if (isRoundTrip) ...[
              ...
              const Gap(10),
              Text(
                "Retour : ${widget.program.villeArrivee} ➔ ${widget.program.villeDepart}",
                style: const TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.bold,
                  color: Colors.grey,
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          "Aucun bus disponible pour ce retour.",
                ...
                Wrap(
                  ...
                  children: availableReturnTrips.map((prog) {
                    final bool isSelected =
                        selectedReturnProgram?.id == prog.id;
                    return ChoiceChip(
                      label: Text(
                        prog.heureDepart,
                        style: TextStyle(
                      ...
                      onSelected: (bool selected) {
                        setState(() {
                          selectedReturnProgram = selected ? prog : null;
                        });
                      },
                    );
                  }).toList(),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: const Color(
                  0xFFCA8A04,
                ).withOpacity(0.1), // Fond Orange subtil
                    ...
                    child: RichText(
                      text: TextSpan(
                        ...
                        children: const [
                          TextSpan(
                            text:
                                "Annulation & Modification possibles jusqu'à ",
                            style: TextStyle(fontWeight: FontWeight.normal),
                          ),
                          TextSpan(
                            text: "15 min avant le départ ",
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
                              color: Color(0xFFCA8A04),
                            ),
                          ...
                          TextSpan(
                            text: "via l'écran 'Mes Réservations'.",
                            style: TextStyle(fontWeight: FontWeight.normal),
                          ),
                        ],
                      ),
