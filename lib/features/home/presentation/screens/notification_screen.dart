...
class _NotificationScreenState extends State<NotificationScreen> with SingleTickerProviderStateMixin { // 👈 AJOUT DU MIXIN
  ...
  void initState() {
    ...
    // On attend que le premier "build" soit terminé avant de charger les données
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _loadData();
    });
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      print("🕵️ --- DEBUG NOTIFICATIONS ---");
      print("🕵️ Token utilisateur : ${token != null ? 'Présent' : 'NUL ❌'}");

      // 1. Charger les messages chauffeur
      try {
        await driverProvider.loadMessages();
        print("🕵️ Messages Chauffeur chargés : ${driverProvider.messages.length}");
      } catch (e) {
        print("🕵️ ❌ Erreur API Messages Chauffeur: $e");
      }

      // 2. Charger les notifications générales
      if (token != null && token.isNotEmpty) {
        final dio = Dio(BaseOptions(
          baseUrl: ApiConfig.baseUrl,
          headers: {
        ...
        try {
          final generalNotifs = await repository.getNotifications();
          _notifications = generalNotifs;
          print("🕵️ Notifications générales chargées : ${_notifications.length}");
        } catch (e) {
          print("🕵️ ❌ Erreur API Notifications générales: $e");
        }
      } else {
        print("🕵️ ⚠️ Pas de token, impossible de charger les notifs générales.");
      }

      if (mounted) {
        setState(() {
          _driverMessages = driverProvider.messages;
          _isLoading = false;
        });
        _entranceController.forward(from: 0.0);
        print("🕵️ Affichage final -> Total items: ${_notifications.length + _driverMessages.length}");
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoading = false);
        print("🕵️ ❌ Erreur globale fatale : $e");
      }
    }
  }
