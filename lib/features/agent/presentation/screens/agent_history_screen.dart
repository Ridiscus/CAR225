...
class _AgentHistoryScreenState extends State<AgentHistoryScreen> {
  ...
  // --- ÉTATS ---
  List<TicketScan> _scans = [];
  bool _isLoading = true;
  String? _errorMessage;

  // 💡 Initialise ton Repository ici (à adapter si tu utilises un Provider ou GetIt)
  ...
  Future<void> _fetchHistory() async {
    ...
    try {
      final results = await _repository.getScanHistory(date: _selectedDate);
      if (mounted) {
        setState(() {
          _scans = results;
          _isLoading = false;
      ...
      if (mounted) {
        setState(() {
          _errorMessage = "Impossible de charger l'historique : $e";
          _isLoading = false;
        });
      }
    }
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (pickedDate != null) {
      ...
      // 🔄 On recharge l'API avec la nouvelle date !
      _fetchHistory();
    }
  }

  // Permet de retirer le filtre et recharger
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // --- HEADER ---
            _buildCompactHeader(_scans.length),
        ...
        child: Row(
          children: [
            ...
            if (_selectedDate != null)
              GestureDetector(
                onTap: _clearDateFilter, // 🔄 On appelle notre nouvelle méthode ici
                child: Container(
              ...
              const Icon(Icons.calendar_month_rounded, color: Color(0xFFCBD5E1), size: 20),
          ],
        ),
      ),
    );
  ...
  // Widget d'erreur au cas où l'API plante
