...
class _BookingSummaryScreenState extends State<BookingSummaryScreen> {
  ...
  Future<void> _processPayment(String methodKey) async {
    ...
    try {
      ...
      finalData['date_voyage'] = rawDate.contains('T')
          ? rawDate.split('T')[0]
          : rawDate.split(' ')[0];

      if (finalData['date_retour'] != null) {
        String rawRetour = finalData['date_retour'].toString();
        finalData['date_retour'] = rawRetour.contains('T')
            ? rawRetour.split('T')[0]
            : rawRetour.split(' ')[0];
      }
      ...
      if (finalData['is_aller_retour'] == true ||
          finalData['is_aller_retour'] == 'true') {
      ...
      for (var p in passengers) {
        ...
        if (finalData['is_aller_retour'] == 1) {
          print(
            "👮 Passager ${pMap['nom']} - Siège Aller: ${pMap['seat_number']} | Retour: ${pMap['seat_number_return']}",
          );
        }
      ...
      // 4. Paiement
      finalData['payment_method'] = methodKey.toLowerCase() == 'carpay'
          ? 'wallet'
          : 'cinetpay';
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        if (response.data['requires_payment'] == true &&
      ...
      if (e is DioException && e.response != null) {
        print(
          "❌ RÉPONSE ERREUR : ${e.response?.data}",
        ); // Regarde ici si l'API te renvoie une erreur précise
    ...
    showModalBottomSheet(
      ...
      builder: (modalContext) {
        return Container(
          ...
          child: SafeArea(
            // ✅ AJOUT DU SAFE AREA ICI
            top: false, // On ne touche pas au haut
            child: Column(
              mainAxisSize:
                  MainAxisSize.min, // S'adapte à la hauteur du contenu
              ...
              children: [
                      ...
                      child: ListTile(
                        ...
                        onTap: hasEnoughFunds
                            ? () {
                                Navigator.pop(
                                  modalContext,
                                ); // Ferme le modal avant de lancer le paiement
                        ...
                        subtitle: Text(
                          hasEnoughFunds
                              ? "Solde: ${_formatCurrency(currentBalance)}"
                              : "Insuffisant (${_formatCurrency(currentBalance)})",
                          style: TextStyle(
                ...
                const Gap(
                  20,
                ), // Un petit espace en bas avant la fin du SafeArea
              ],
            ),
          ),
        );
  ...
  Widget build(BuildContext context) {
    // Calcul Prix
    final int ticketPrice = widget.program.isAllerRetour
        ? (widget.program.prix * 2)
        : widget.program.prix;
    final int passengerCount = (widget.bookingData['nombre_places'] as num)
        .toInt();
    ...
    // Date Départ
    String rawDateDepart =
        widget.bookingData['date_voyage']?.toString() ??
        widget.program.dateDepart;
    if (rawDateDepart.contains('T'))
      rawDateDepart = rawDateDepart.split('T')[0];
    DateTime dateD = DateTime.tryParse(rawDateDepart) ?? DateTime.now();
    String formattedDateDepart = DateFormat(
      "EEE d MMM yyyy",
      "fr_FR",
    ).format(dateD);
    ...
    if (widget.program.isAllerRetour &&
        widget.bookingData['date_retour'] != null) {
      String rawDateRetour = widget.bookingData['date_retour'].toString();
      if (rawDateRetour.contains('T'))
        rawDateRetour = rawDateRetour.split('T')[0];
          ...
          child: Container(
            ...
            child: ElevatedButton(
              ...
              onPressed: isSubmitting
                  ? null
                  : () => _showPaymentMethodSelector(context, total),
              ...
              style: ElevatedButton.styleFrom(
                ...
                disabledBackgroundColor:
                    Colors.transparent, // Même désactivé, on voit l'image !
          ...
          children: [
            ...
            _buildSectionCard(
              ...
              children: [
                ...
                if (widget.program.isAllerRetour &&
                    formattedDateRetour.isNotEmpty) ...[
                  const Gap(10),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                ...
                ...List.generate(passengerCount, (index) {
                  final List rawPassengers =
                      widget.bookingData['passagers'] as List;
                  final p = rawPassengers[index] as Map<String, dynamic>;

                  // Récupération des sièges
                  final seatAller = p['seat_number'];
                  final seatRetour =
                      p['seat_number_return']; // Peut être null si aller simple
