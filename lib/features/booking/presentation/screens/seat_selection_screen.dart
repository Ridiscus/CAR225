...
class _SeatSelectionScreenState extends State<SeatSelectionScreen> {
  ...
  Set<int> get currentSelectedSeats =>
      isSelectingReturnPhase ? selectedSeatsRetour : selectedSeatsAller;
  ...
  Future<void> _fetchReservedSeats() async {
    setState(() {
      isLoadingSeats = true;
      occupiedSeats =
          []; // On vide pour éviter d'afficher les sièges occupés de l'aller sur le retour
    });
    ...
    try {
      ...
      if (isSelectingReturnPhase) {
        ...
        targetDateStr =
            widget.dateRetourChoisie ?? targetProgram.dateDepart.split(' ')[0];
      } else {
        // C'est l'aller
        targetProgram = widget.program;
        targetDateStr =
            targetProgram.dateDepart; // DateTime.parse gérera l'heure
      }

      // 2. FORMATAE DATE
      DateTime dateObj = DateTime.parse(
        targetDateStr.contains(' ') ? targetDateStr : "$targetDateStr 00:00:00",
      );
      String datePropre = DateFormat('yyyy-MM-dd').format(dateObj);

      print(
        "🔍 REQUÊTE SIÈGES (${isSelectingReturnPhase ? 'RETOUR' : 'ALLER'}) -> ID: ${targetProgram.id}, DATE: $datePropre",
      );
  ...
  void _handleMainButtonPress() {
    ...
    if (currentSelectedSeats.length != widget.passengerCount) {
      _showTopNotification(
        "Veuillez sélectionner ${widget.passengerCount} siège(s).",
      );
      return;
    }
    ...
    if (widget.returnProgram != null && !isSelectingReturnPhase) {
      ...
      _showTopNotification(
        "Sélectionnez maintenant vos places pour le RETOUR",
        isError: false,
      );
    }
              ...
              children: [
                  ...
                  child: Form(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final seatRetour =
                            (widget.returnProgram != null &&
                                sortedSeatsRetour.length > index)
                ...
                SafeArea(
                  child: Container(
                    padding: const EdgeInsets.all(20),
                    child: Container(
                      // On remplace le SizedBox par Container
  ...
  void _submitDataAndNavigate() {
    ...
    if (selectedSeatsAller.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Veuillez sélectionner au moins une place aller."),
        ),
      );
      return;
    }
    ...
    if (widget.isModificationMode) {
      ...
      if (widget.program.isAllerRetour && widget.returnProgram != null) {
        ...
        modificationData["return_heure_depart"] =
            widget.returnProgram!.heureDepart;
    ...
    final bookingData = {
      ...
      if (widget.returnProgram != null)
        "return_programme_id": widget.returnProgram!.id,
  ...
  Widget build(BuildContext context) {
    ...
    final currentProgram = isSelectingReturnPhase
        ? widget.returnProgram!
        : widget.program;
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(
              "${currentProgram.compagnieName} • ${widget.passengerCount} passager(s)",
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(children: [_buildBusLayout(context)]),
                    ),
                  ],
                ),
              ),
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              ...
              disabledBackgroundColor:
                  Colors.transparent, // Crucial pour le gris
              shadowColor: Colors.transparent,
              elevation: 0,
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(15),
