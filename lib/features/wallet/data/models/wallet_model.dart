...
class WalletTransaction {
  ...
  factory WalletTransaction.fromJson(Map<String, dynamic> json) {
    return WalletTransaction(
      titre: json['description'] ?? "Transaction",
      montant: json['amount'].toString(),
      date: json['created_at'] ?? DateTime.now().toString(),
      isCredit: json['type'] == 'credit',
      // 🟢 NOUVEAU : On récupère le statut (completed, pending, failed)
      // Par sécurité, si l'API ne renvoie rien, on considère que c'est en attente (pending)
