/*import 'package:car225/features/home/presentation/screens/personal_info_screen.dart';
import 'package:car225/features/home/presentation/screens/security_screen.dart';
import 'package:car225/features/home/presentation/screens/wallet_screen.dart';
import 'package:flutter/material.dart';
import 'package:gap/gap.dart';
import 'package:flutter_svg/flutter_svg.dart';

import '../../../../core/services/device/device_service.dart';
import '../../../../core/services/notifications/fcm_service.dart';
import '../../../auth/data/datasources/auth_remote_data_source.dart';
import '../../../auth/data/repositories/auth_repository_impl.dart';
import '../../../auth/presentation/screens/login_screen.dart';
import 'account_setting_screen.dart';
import 'package:car225/features/auth/presentation/screens/login_screen.dart';
// Assure-toi d'importer tes couleurs globales si besoin
// import '../../../../core/theme/app_colors.dart';

class ProfileScreen extends StatelessWidget {
  const ProfileScreen({super.key});

  @override
  Widget build(BuildContext context) {
    // --- 1. THEME VARIABLES ---
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final scaffoldColor = Theme.of(context).scaffoldBackgroundColor;
    final cardColor = Theme.of(context).cardColor;
    final textColor = Theme.of(context).textTheme.bodyLarge?.color;
    // Couleur secondaire (pour les sous-textes)
    final secondaryTextColor = isDark ? Colors.grey[400] : Colors.grey;

    return Scaffold(
      backgroundColor: scaffoldColor, // <--- FOND DYNAMIQUE
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        automaticallyImplyLeading: false,
      ),
      body: SingleChildScrollView(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          children: [
            // --- 2. PHOTO DE PROFIL + BADGE ---
            Center(
              child: Stack(
                children: [
                  // La photo
                  Container(
                    width: 100,
                    height: 100,
                    decoration: BoxDecoration(
                      color: cardColor,
                      borderRadius: BorderRadius.circular(25),
                      // Bordure : Gris clair le jour, gris foncé la nuit
                      border: Border.all(color: isDark ? Colors.grey[800]! : Colors.grey.shade300, width: 3),
                      image: const DecorationImage(
                        image: AssetImage("assets/images/buse.png"),
                        fit: BoxFit.cover,
                      ),
                    ),
                  ),
                  // Le petit bouton "+"
                  Positioned(
                    bottom: 0,
                    right: 0,
                    child: Container(
                      height: 32,
                      width: 32,
                      decoration: BoxDecoration(
                          color: cardColor, // <--- DYNAMIQUE
                          shape: BoxShape.circle,
                          border: Border.all(color: isDark ? Colors.grey[700]! : Colors.grey.shade300),
                          boxShadow: [
                            BoxShadow(
                                color: isDark ? Colors.black26 : Colors.black.withOpacity(0.1),
                                blurRadius: 5
                            )
                          ]
                      ),
                      // Icone noire le jour, blanche la nuit
                      child: Icon(Icons.add, size: 18, color: textColor),
                    ),
                  )
                ],
              ),
            ),
            const Gap(15),

            // --- 3. NOM & STATUT ---
            Text(
              "Eyes Kouassi",
              style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: textColor), // <--- DYNAMIQUE
            ),
            const Gap(5),
            Text(
              "MEMBRE CAR 225 GOLD",
              style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: secondaryTextColor, letterSpacing: 1.0),
            ),
            const Gap(25),

            // --- 4. STATISTIQUES (Voyages & Points) ---
            Row(
              children: [
                Expanded(
                  child: _buildStatCard(context, "12", "VOYAGES"),
                ),
                const Gap(15),
                Expanded(
                  child: _buildStatCard(context, "2.25K", "POINTS", isPoints: true),
                ),
              ],
            ),
            const Gap(25),

            // --- 5. LISTE DES OPTIONS ---
            _buildMenuOption(
              context, // <--- On passe le context
              icon: Icons.person_outline,
              title: "Mes informations personnelles",
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => const PersonalInfoScreen()));
              },
            ),
            const Gap(15),

            _buildMenuOption(
              context,
              icon: Icons.account_balance_wallet_outlined,
              title: "Portefeuille Mobile Money",
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => const WalletScreen()));
              },
            ),
            const Gap(15),

            _buildMenuOption(
              context,
              icon: Icons.settings_outlined,
              title: "Paramètre du compte",
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => const AccountSettingsScreen()));
              },
            ),
            const Gap(15),

            _buildMenuOption(
              context,
              icon: Icons.shield_outlined,
              title: "Sécurité & Confidentialité",
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => const SecurityScreen()));
              },
            ),
            const Gap(15),

            // Bouton Déconnexion
            _buildMenuOption(
              context,
              icon: Icons.login_outlined,
              title: "Déconnexion",
              textColor: Colors.red,
              iconColor: isDark ? Colors.white70 : Colors.black, // Ajustement icone déconnexion
              onTap: () {
                _showLogoutDialog(context);
              },
            ),

            const Gap(120),
          ],
        ),
      ),
    );
  }

  // WIDGET : Carte de Statistique
  Widget _buildStatCard(BuildContext context, String value, String label, {bool isPoints = false}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;

    // Logique des couleurs pour les stats
    // Jour : Fond Beige (FFF3E0). Nuit : Fond Gris foncé transparent (White10) ou Noir très léger
    final bgColor = isDark ? Colors.white10 : const Color(0xFFFFF3E0);
    final borderColor = Colors.orange.withOpacity(0.1);
    final valueColor = isDark ? Colors.white : Colors.black;
    final labelColor = isDark ? Colors.grey[400] : Colors.black54;

    return Container(
      padding: const EdgeInsets.symmetric(vertical: 20),
      decoration: BoxDecoration(
        color: bgColor,
        borderRadius: BorderRadius.circular(15),
        border: Border.all(color: borderColor),
      ),
      child: Column(
        children: [
          Text(
            value,
            style: TextStyle(
              fontSize: 24,
              fontWeight: FontWeight.w900,
              color: valueColor,
            ),
          ),
          const Gap(5),
          Text(
            label,
            style: TextStyle(
              fontSize: 10,
              fontWeight: FontWeight.bold,
              color: labelColor,
            ),
          ),
        ],
      ),
    );
  }

  // WIDGET : Option de Menu
  Widget _buildMenuOption(
      BuildContext context, {
        required IconData icon,
        required String title,
        required VoidCallback onTap,
        Color? textColor, // Optionnel, si null on calcule dynamiquement
        Color? iconColor, // Optionnel
      }) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final cardColor = Theme.of(context).cardColor;
    final defaultTextColor = Theme.of(context).textTheme.bodyLarge?.color;

    // Bordures et Ombres
    final borderColor = isDark ? Colors.grey[800]! : Colors.grey.shade200;
    final shadowColor = isDark ? Colors.transparent : Colors.black.withOpacity(0.02);

    // Couleurs finales
    final finalTextColor = textColor ?? (isDark ? Colors.grey[300] : Colors.grey.shade600);
    final finalIconColor = iconColor ?? (isDark ? Colors.grey[400] : Colors.grey);
    final arrowColor = isDark ? Colors.grey[600] : Colors.grey.shade400;

    return Container(
      decoration: BoxDecoration(
        color: cardColor, // <--- FOND DYNAMIQUE
        borderRadius: BorderRadius.circular(15),
        boxShadow: [
          BoxShadow(
            color: shadowColor,
            blurRadius: 10,
            offset: const Offset(0, 2),
          ),
        ],
        border: Border.all(color: borderColor),
      ),
      child: Material(
        color: Colors.transparent,
        borderRadius: BorderRadius.circular(15),
        child: InkWell(
          onTap: onTap,
          borderRadius: BorderRadius.circular(15),
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 18),
            child: Row(
              children: [
                Icon(icon, color: finalIconColor, size: 22),
                const Gap(15),

                Expanded(
                  child: Text(
                    title,
                    style: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w600,
                      color: finalTextColor,
                    ),
                  ),
                ),

                Icon(
                  Icons.arrow_forward_ios_rounded,
                  size: 16,
                  color: arrowColor,
                ),
              ],
            ),
          ),
        ),
      ),
    );
  }



  void _showLogoutDialog(BuildContext parentContext) { // 1. On renomme ici "parentContext" pour être clair
    showDialog(
      context: parentContext,
      builder: (dialogContext) => AlertDialog( // 2. On renomme ici "dialogContext"
        title: const Text("Déconnexion"),
        content: const Text("Voulez-vous vraiment vous déconnecter ?"),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(dialogContext), // On ferme avec dialogContext
            child: const Text("Annuler", style: TextStyle(color: Colors.grey)),
          ),
          TextButton(
            onPressed: () async {
              // 1. On ferme le dialog (ce contexte meurt ici)
              Navigator.pop(dialogContext);

              try {
                final authRepository = AuthRepositoryImpl(
                  remoteDataSource: AuthRemoteDataSourceImpl(),
                  fcmService: FcmService(),
                  deviceService: DeviceService(),
                );

                // 2. Traitement async
                await authRepository.logout();

                // 3. ICI LA CORRECTION :
                // On vérifie si "parentContext" (l'écran derrière) est toujours là.
                // Comme on n'a fermé que le dialog, l'écran derrière est toujours monté !
                if (!parentContext.mounted) return;

                // 4. On navigue en utilisant le contexte du PARENT
                Navigator.pushAndRemoveUntil(
                  parentContext,
                  MaterialPageRoute(builder: (context) => const LoginScreen()),
                      (route) => false,
                );

              } catch (e) {
                debugPrint("Erreur déconnexion: $e");

                if (!parentContext.mounted) return;
                Navigator.pushAndRemoveUntil(
                  parentContext,
                  MaterialPageRoute(builder: (context) => const LoginScreen()),
                      (route) => false,
                );
              }
            },
            child: const Text("Déconnexion", style: TextStyle(color: Colors.red)),
          ),
        ],
      ),
    );
  }

}*/



import 'package:flutter/material.dart';
import 'package:gap/gap.dart';

import '../../../../core/services/device/device_service.dart';
import '../../../../core/services/notifications/fcm_service.dart';
import '../../../auth/data/datasources/auth_remote_data_source.dart';
import '../../../auth/data/repositories/auth_repository_impl.dart';
import '../../../auth/presentation/screens/login_screen.dart';
import '../../../auth/data/models/user_model.dart'; // Import du modèle
import 'personal_info_screen.dart';
import 'security_screen.dart';
import 'wallet_screen.dart';
import 'account_setting_screen.dart';

// ⚠️ TRANSFORMATION EN STATEFUL WIDGET POUR GERER LE CHARGEMENT
class ProfileScreen extends StatefulWidget {
  const ProfileScreen({super.key});
  @override
  State<ProfileScreen> createState() => _ProfileScreenState();
}

class _ProfileScreenState extends State<ProfileScreen> {
  // Repo
  late AuthRepositoryImpl _authRepository;

  // Variables d'état
  UserModel? _user;
  bool _isLoading = true;

  @override
  void initState() {
    super.initState();
    // Init du repo
    _authRepository = AuthRepositoryImpl(
      remoteDataSource: AuthRemoteDataSourceImpl(),
      fcmService: FcmService(),
      deviceService: DeviceService(),
    );
    // Chargement initial
    _fetchUserProfile();
  }

  // Fonction pour récupérer les infos
  Future<void> _fetchUserProfile() async {
    setState(() => _isLoading = true);
    try {
      final user = await _authRepository.getUserProfile();
      if (mounted) {
        setState(() {
          _user = user;
          _isLoading = false;
        });
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoading = false);
        // Tu peux afficher un petit message d'erreur discret si tu veux
        print("Erreur chargement profil: $e");
      }
    }
  }

  // Fonction navigation intelligente
  void _navigateToEdit() async {
    // On attend le retour de la page de modification
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => const PersonalInfoScreen()),
    );
    // QUAND ON REVIENT ICI, ON RECHARGE LES DONNÉES !
    _fetchUserProfile();
  }

  @override
  Widget build(BuildContext context) {
    
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final scaffoldColor = Theme.of(context).scaffoldBackgroundColor;
    final cardColor = Theme.of(context).cardColor;
    final textColor = Theme.of(context).textTheme.bodyLarge?.color;
    final secondaryTextColor = isDark ? Colors.grey[400] : Colors.grey;

    // Gestion du chargement
    if (_isLoading) {
      return Scaffold(backgroundColor: scaffoldColor, body: const Center(child: CircularProgressIndicator()));
    }

    return Scaffold(
      backgroundColor: scaffoldColor,
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        automaticallyImplyLeading: false,
      ),
      body: SingleChildScrollView(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          children: [
            // --- 2. PHOTO DE PROFIL DYNAMIQUE ---
            Center(
              child: Stack(
                children: [
                  Container(
                    width: 100,
                    height: 100,
                    decoration: BoxDecoration(
                      color: cardColor,
                      borderRadius: BorderRadius.circular(25),
                      border: Border.all(color: isDark ? Colors.grey[800]! : Colors.grey.shade300, width: 3),
                      image: DecorationImage(
                        fit: BoxFit.cover,
                        // LOGIQUE D'IMAGE :
                        image: (_user?.photoUrl != null && _user!.photoUrl!.isNotEmpty)
                            ? NetworkImage(_user!.photoUrl!) as ImageProvider
                            : const AssetImage("assets/images/buse.png"), // Image par défaut
                      ),
                    ),
                  ),
                  // Le petit bouton "+" (Redirige vers edit aussi)
                  Positioned(
                    bottom: 0,
                    right: 0,
                    child: GestureDetector(
                      onTap: _navigateToEdit,
                      child: Container(
                        height: 32, width: 32,
                        decoration: BoxDecoration(
                            color: cardColor,
                            shape: BoxShape.circle,
                            border: Border.all(color: isDark ? Colors.grey[700]! : Colors.grey.shade300),
                            boxShadow: [BoxShadow(color: isDark ? Colors.black26 : Colors.black.withOpacity(0.1), blurRadius: 5)]
                        ),
                        child: Icon(Icons.edit, size: 16, color: textColor), // J'ai mis un crayon (edit) c'est plus logique
                      ),
                    ),
                  )
                ],
              ),
            ),
            const Gap(15),

            // --- 3. NOM DYNAMIQUE ---
            Text(
              _user != null ? "${_user!.prenom} ${_user!.name}" : "Utilisateur",
              style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: textColor),
            ),
            const Gap(5),
            Text(
              "MEMBRE CAR 225", // Tu pourras rendre ça dynamique plus tard si l'API renvoie le statut
              style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: secondaryTextColor, letterSpacing: 1.0),
            ),
            const Gap(25),

            // --- 4. STATISTIQUES (Statiques pour l'instant) ---
            Row(
              children: [
                Expanded(child: _buildStatCard(context, "12", "VOYAGES")),
                const Gap(15),
                Expanded(child: _buildStatCard(context, "2.25K", "POINTS", isPoints: true)),
              ],
            ),
            const Gap(25),

            // --- 5. LISTE DES OPTIONS ---
            _buildMenuOption(
              context,
              icon: Icons.person_outline,
              title: "Mes informations personnelles",
              onTap: _navigateToEdit, // Utilise la fonction qui recharge au retour
            ),
            const Gap(15),

            _buildMenuOption(
              context,
              icon: Icons.account_balance_wallet_outlined,
              title: "Portefeuille Mobile Money",
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const WalletScreen())),
            ),
            const Gap(15),

            _buildMenuOption(
              context,
              icon: Icons.settings_outlined,
              title: "Paramètre du compte",
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const AccountSettingsScreen())),
            ),
            const Gap(15),

            _buildMenuOption(
              context,
              icon: Icons.shield_outlined,
              title: "Sécurité & Confidentialité",
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const SecurityScreen())),
            ),
            const Gap(15),

            // Bouton Déconnexion
            _buildMenuOption(
              context,
              icon: Icons.login_outlined,
              title: "Déconnexion",
              textColor: Colors.red,
              iconColor: isDark ? Colors.white70 : Colors.black,
              onTap: () => _showLogoutDialog(context),
            ),

            const Gap(120),
          ],
        ),
      ),
    );
  }

  // --- WIDGETS HELPER (Inchangés) ---
  Widget _buildStatCard(BuildContext context, String value, String label, {bool isPoints = false}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final bgColor = isDark ? Colors.white10 : const Color(0xFFFFF3E0);
    final borderColor = Colors.orange.withOpacity(0.1);
    final valueColor = isDark ? Colors.white : Colors.black;
    final labelColor = isDark ? Colors.grey[400] : Colors.black54;

    return Container(
      padding: const EdgeInsets.symmetric(vertical: 20),
      decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(15), border: Border.all(color: borderColor)),
      child: Column(
        children: [
          Text(value, style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: valueColor)),
          const Gap(5),
          Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: labelColor)),
        ],
      ),
    );
  }

  Widget _buildMenuOption(BuildContext context, {required IconData icon, required String title, required VoidCallback onTap, Color? textColor, Color? iconColor}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final cardColor = Theme.of(context).cardColor;
    final borderColor = isDark ? Colors.grey[800]! : Colors.grey.shade200;
    final finalTextColor = textColor ?? (isDark ? Colors.grey[300] : Colors.grey.shade600);
    final finalIconColor = iconColor ?? (isDark ? Colors.grey[400] : Colors.grey);
    final arrowColor = isDark ? Colors.grey[600] : Colors.grey.shade400;

    return Container(
      decoration: BoxDecoration(color: cardColor, borderRadius: BorderRadius.circular(15), boxShadow: [BoxShadow(color: isDark ? Colors.transparent : Colors.black.withOpacity(0.02), blurRadius: 10, offset: const Offset(0, 2))], border: Border.all(color: borderColor)),
      child: Material(
        color: Colors.transparent, borderRadius: BorderRadius.circular(15),
        child: InkWell(
          onTap: onTap, borderRadius: BorderRadius.circular(15),
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 18),
            child: Row(
              children: [
                Icon(icon, color: finalIconColor, size: 22), const Gap(15),
                Expanded(child: Text(title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: finalTextColor))),
                Icon(Icons.arrow_forward_ios_rounded, size: 16, color: arrowColor),
              ],
            ),
          ),
        ),
      ),
    );
  }

  void _showLogoutDialog(BuildContext parentContext) {
    showDialog(
      context: parentContext,
      builder: (dialogContext) => AlertDialog(
        title: const Text("Déconnexion"),
        content: const Text("Voulez-vous vraiment vous déconnecter ?"),
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text("Annuler", style: TextStyle(color: Colors.grey))),
          TextButton(
            onPressed: () async {
              Navigator.pop(dialogContext);
              try {
                // Pas besoin de recréer le repo, on l'a déjà dans _authRepository
                await _authRepository.logout();
                if (!parentContext.mounted) return;
                Navigator.pushAndRemoveUntil(parentContext, MaterialPageRoute(builder: (context) => const LoginScreen()), (route) => false);
              } catch (e) {
                if (!parentContext.mounted) return;
                Navigator.pushAndRemoveUntil(parentContext, MaterialPageRoute(builder: (context) => const LoginScreen()), (route) => false);
              }
            },
            child: const Text("Déconnexion", style: TextStyle(color: Colors.red)),
          ),
        ],
      ),
    );
  }
}