...
import 'home_tab_screen.dart';
import 'my_tickets_tab_screen.dart';
import 'convoi_tab_screen.dart'; // 👈 NOUVEL IMPORT POUR L'ONGLET CONVOI
import 'companies_tab_screen.dart';
import 'alerts_tab_screen.dart';
...
class _MainScreenState extends State<MainScreen> {
  ...
  // 🟢 On transforme la variable en 'getter' (get) pour pouvoir utiliser setState dedans
  List<Widget> get _pages => [
    const HomeTabScreen(),

    // 🟢 On passe la fonction au MyTicketsTabScreen
    MyTicketsTabScreen(
      onGoHome: () {
        setState(() {
          _currentIndex = 0; // Ramène à l'onglet Accueil
        });
      },
    ),

    const ConvoiTabScreen(),
    const CompaniesTabScreen(),
    const AlertsTabScreen(),
  ];
        ...
        bottomNavigationBar: SafeArea(
          ...
          child: CurvedNavigationBar(
            ...
            items: <Widget>[
              _buildNavItem("assets/icons/home.png", 0, isDark),
              _buildNavItem("assets/icons/ticket.png", 1, isDark),
              _buildNavItem("assets/icons/convoi.png", 2, isDark), // 👈 NOUVELLE ICÔNE POUR LE CONVOI
              _buildNavItem("assets/icons/buss.png", 3, isDark),    // 👈 Index mis à jour à 3
              _buildNavItem("assets/icons/warning.png", 4, isDark), // 👈 Index mis à jour à 4
            ],
            onTap: (index) {
              setState(() {
                _currentIndex = index;
