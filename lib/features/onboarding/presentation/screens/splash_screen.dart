            ...
            children: [
              ...
              Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.yellowAccent.withOpacity(0.7), width: 2)),
                ).animate()
                    .scale(duration: 1000.ms, begin: const Offset(1, 1), end: const Offset(1.15, 1.15), curve: Curves.easeInOut)
                    .tint(color: Colors.white, duration: 1000.ms)
                    .then(delay: 500.ms).fadeOut(duration: 300.ms),
              ),
              ...
              Center(
                child: Container(
                  ...
                  decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle),
                    ...
                    .then().scale(delay: 500.ms, duration: 800.ms, end: const Offset(30, 30), curve: Curves.easeInExpo),
              ),

              // --- 2. LE LOGO "A" ---
              Center(
                child: SvgPicture.asset('assets/vectors/logo_A.svg', height: 60)
                    .animate().fadeIn(duration: 500.ms).then(delay: 1000.ms).fadeOut(duration: 300.ms, curve: Curves.easeOut),
              ),

              // --- 3. LA PARENTHÈSE / ARCHE (Partie HAUTE) ---
              Positioned(
                top: size.height * 0.50 - 35, left: 0, right: 0,
                child: Center(child: SvgPicture.asset('assets/vectors/logo_text_top.svg', height: 30)),
              ).animate(delay: startTextDelay.ms)
                  .moveY(begin: -250, end: 0, duration: moveDuration.ms, curve: Curves.easeOutQuart)
                  .fadeIn(duration: 200.ms)
                  .then(delay: pauseAuCentre.ms)
                  .moveY(begin: 0, end: ascensionDistance, duration: ascensionDuration.ms, curve: Curves.easeInOutCubic),

              // --- 4. LE TEXTE "CAR 225" (Partie BASSE) ---
              Positioned(
                top: size.height * 0.50 - 5, left: 0, right: 0,
                child: Center(child: SvgPicture.asset('assets/vectors/logo_text_bottom.svg', height: 70)),
              ).animate(delay: startTextDelay.ms)
                  .moveY(begin: 250, end: 0, duration: moveDuration.ms, curve: Curves.easeOutQuart)
                  .fadeIn(duration: 200.ms)
                  .then(delay: pauseAuCentre.ms)
...
import 'onboarding_screen.dart'; // <--- Ton écran d'accueil si non connecté
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void _handleNavigation() {
    ...
    // 🟢 CHOIX DE LA PAGE DE DESTINATION
    Widget destination = const OnboardingScreen(); // Par défaut : non connecté

    if (_isConnected) {
      if (_userRole == 'hotesse') {
  ...
  @override
  Widget build(BuildContext context) {
    // Ta logique UI reste EXACTEMENT la même
    final size = MediaQuery.of(context).size;
    const int startTextDelay = 2000;
    const int moveDuration = 800;
    const int pauseAuCentre = 600;
