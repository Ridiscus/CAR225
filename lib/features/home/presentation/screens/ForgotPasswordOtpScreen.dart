...
class _ForgotPasswordOtpScreenState extends State<ForgotPasswordOtpScreen> {
  ...
  void _verifyCode() async {
    ...
    if (_code.length != _otpLength) {
      print(
        "❌ [DEBUG] Validation échouée : Le code ne fait pas $_otpLength chiffres.",
      );
      _showTopNotification(
        "Veuillez entrer les $_otpLength chiffres",
        isError: true,
      );
    ...
    try {
      ...
      // Appel API (Nouveau)
      await context.read<AuthRepository>().verifyPasswordOtp(
        widget.email,
        _code,
      );
      ...
      showDialog(
        ...
        builder: (context) => const Dialog(
          ...
          elevation: 0,
          child: Center(child: _SuccessAnimationWidget()),
        ),
      );
      ...
      if (mounted) {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) =>
                ForgotPasswordResetScreen(email: widget.email, otpCode: _code),
          ),
        );
      }
    } catch (e, stackTrace) {
